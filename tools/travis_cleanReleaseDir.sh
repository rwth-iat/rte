...
mv -v `find . -maxdepth 1  -name '*acpltRTE-linux32.tar.gz' -type f -printf '%p\n' | sort -n | tail -10 | cut -f2- -d" " ` tmpSafe/.
mv -v `find . -maxdepth 1  -name '*acpltRTE-linux64.tar.gz' -type f -printf '%p\n' | sort -n | tail -10 | cut -f2- -d" " ` tmpSafe/.
mv -v `find . -maxdepth 1  -name '*acpltRTE-win32.zip' -type f -printf '%p\n' | sort -n | tail -10 | cut -f2- -d" " ` tmpSafe/.
mv -v `find . -maxdepth 1  -name '*acpltRTE-RPi.tar.gz' -type f -printf '%p\n' | sort -n | tail -10 | cut -f2- -d" " ` tmpSafe/.
