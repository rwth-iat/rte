...
OV_DLLFNCEXPORT UA_StatusCode opcua_nodeStoreFunctions_ovReferenceTypeNodeToOPCUA(
		void *handle, OV_INSTPTR_opcua_referenceTypeNode ovNode, UA_ReferenceTypeNode** opcuaNode) {
	OV_INSTPTR_ov_object pchild = NULL;
	*opcuaNode = NULL;
	*opcuaNode = ov_database_malloc(sizeof(UA_ReferenceTypeNode));
	if (*opcuaNode == NULL)
		return 99;
	...
	(*opcuaNode)->isAbstract = ovNode->v_isAbstract;
	(*opcuaNode)->symmetric = ovNode->v_symmetric;
	copyOvLocalizedTextToOPCUA	(&ovNode->p_inverseName	, &(*opcuaNode)->inverseName);
