...
SRV_extAny_t* SRV_extAny_t_new(){
	SRV_extAny_t* this = malloc(sizeof(SRV_extAny_t));
	SRV_extAny_t_init(this);
	return this;
}
...
void* SRV_serviceGeneric_new(SRV_service_t type){
	...
	case SRV_createSubModelReq:
		return createSubModelReq_t_new();
	case SRV_createSubModelRsp:
		return createSubModelRsp_t_new();
	case SRV_deleteSubModelReq:
		return deleteSubModelReq_t_new();
	case SRV_deleteSubModelRsp:
		return deleteSubModelRsp_t_new();
...
createSubModelReq_t* createSubModelReq_t_new(){
	createSubModelReq_t* this = malloc(sizeof(createSubModelReq_t));
	createSubModelReq_t_init(this);
	return this;
}

createSubModelRsp_t* createSubModelRsp_t_new(){
	createSubModelRsp_t* this = malloc(sizeof(createSubModelRsp_t));
	createSubModelRsp_t_init(this);
	return this;
}
...
deleteSubModelReq_t* deleteSubModelReq_t_new(){
	deleteSubModelReq_t* this = malloc(sizeof(deleteSubModelReq_t));
	deleteSubModelReq_t_init(this);
	return this;
}
deleteSubModelRsp_t* deleteSubModelRsp_t_new(){
	deleteSubModelRsp_t* this = malloc(sizeof(deleteSubModelRsp_t));
	deleteSubModelRsp_t_init(this);
	return this;
}
...
void SRV_extAny_t_init(SRV_extAny_t* this){
	if(!this)
		return;
	memset(&this->value,0,sizeof(SRV_Any_t));
	this->type = SRV_VT_undefined;
	this->time = 0;
	this->hasTime = 0;
}
...
void PVSL_t_init (PVSL_t* this){
	...
	SRV_ident_t_init(&this->propertyId);
	SRV_String_init(&this->preferredName);
	this->pvs = NULL;
	this->numPvs = 0;
	this->hasName = false;
...
void createSubModelReq_t_init(createSubModelReq_t* this){
	...
	this->hasRevision = 0;
	this->hasVersion = 0;
	SRV_ident_t_init(&this->modelId);
	SRV_String_init(&this->name);
	SRV_ident_t_init(&this->parentId);
	this->revision = 0;
	this->version = 0;
}
...
void SRV_extAny_t_deleteMembers (SRV_extAny_t* this){
	if(this->type==SRV_VT_STRING)
		SRV_String_deleteMembers(&this->value.vt_string);
	this->type = SRV_VT_undefined;
}
...
void PVSL_t_deleteMembers (PVSL_t* this){
	...
	SRV_ident_t_deleteMembers(&this->propertyId);
	SRV_String_deleteMembers(&this->preferredName);
	if(this->pvs){
		for(uint32_t i = 0; i<this->numPvs; i++){
...
void PVS_t_deleteMembers (PVS_t* this){
	...
	SRV_ident_t_deleteMembers(&this->carrierId);
	SRV_ident_t_deleteMembers(&this->propertyId);
	SRV_extAny_t_deleteMembers(&this->value);
}
...
void SRV_serviceGeneric_deleteMembers(void* this, SRV_service_t type){
	...
	case SRV_deleteAASRsp:
		...
		break;
	case SRV_createSubModelReq:
		createSubModelReq_t_deleteMembers(this);
		break;
	case SRV_createSubModelRsp:
		//createSubModelRsp_t_deleteMembers(this);
		break;
	case SRV_deleteSubModelReq:
		deleteSubModelReq_t_deleteMembers(this);
		break;
	case SRV_deleteSubModelRsp:
		//deleteSubModelRsp_t_deleteMembers(this);
		break;
	case SRV_createPVSLReq:
...
void createSubModelReq_t_deleteMembers(createSubModelReq_t* this){
	if(!this)
		return;
	SRV_ident_t_deleteMembers(&this->modelId);
	SRV_String_deleteMembers(&this->name);
	SRV_ident_t_deleteMembers(&this->parentId);
}
...
void createPVSLReq_t_deleteMembers (createPVSLReq_t* this){
	if(!this)
		return;
	SRV_ident_t_deleteMembers(&this->parentId);
	PVSL_t_deleteMembers(&this->pvsl);
}
...
void createPVSReq_t_deleteMembers (createPVSReq_t* this){
	if(!this)
		return;
	SRV_ident_t_deleteMembers(&this->listId);
...
void getCoreDataRsp_t_deleteMembers(getCoreDataRsp_t* this){
	if(!this)
		return;
	if(this->pvsl){
		for(uint32_t i = 0; i<this->numPvsl; i++){
...
void SRV_extAny_t_delete (SRV_extAny_t* this){
	SRV_extAny_t_deleteMembers(this);
	free(this);
