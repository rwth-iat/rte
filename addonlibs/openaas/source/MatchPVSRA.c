...
OV_DLLFNCEXPORT OV_UINT openaas_MatchPVSRA_matchPVSRA(OV_STRING requirementPath, OV_STRING assurancePath, OV_BOOL *match, OV_STRING *matchText, OV_STRING *errorText) {
	...
	if (!Ov_CanCastTo(openaas_PropertyValueStatement, pobj)){
	...
	if (!Ov_CanCastTo(openaas_PropertyValueStatement, pobj2)){
		ov_string_setvalue(errorText, "assurancePath is not a path to an PropertyValueStatement");
		return 1;
	}

	requirement = Ov_DynamicPtrCast(openaas_PropertyValueStatement, pobj);
	assurance = Ov_DynamicPtrCast(openaas_PropertyValueStatement, pobj2);
...
OV_DLLFNCEXPORT void openaas_MatchPVSRA_typemethod(
    ...
    *   local variables
    */
    OV_INSTPTR_openaas_MatchPVSRA pinst = Ov_StaticPtrCast(openaas_MatchPVSRA, pfb);
    pinst->v_error = 0;
