...
static OV_RESULT copyOvStringToOPCUA(OV_STRING src, UA_String* dst) {
	if(!src)
		return OV_ERR_OK;
	size_t len = ov_string_getlength(src);
	dst->data = ov_database_malloc(len);
	if (dst->data == NULL)
...
OV_RESULT copyOvNodeIdToOPCUA(OV_INSTPTR_opcua_nodeId src, UA_NodeId *dst) {
	dst->namespaceIndex = src->v_namespaceIndex;
	switch (src->v_identifierType) {
	case UA_NODEIDTYPE_NUMERIC:
		dst->identifierType = UA_NODEIDTYPE_NUMERIC;
		dst->identifier.numeric = src->v_id.value.valueunion.val_uint;
		break;
	case UA_NODEIDTYPE_STRING:
		dst->identifierType = UA_NODEIDTYPE_STRING;
		copyOvStringToOPCUA(src->v_id.value.valueunion.val_string,
...
OV_RESULT copyOvExpandedNodeIdToOPCUA(OV_INSTPTR_opcua_expandedNodeId src,
	...
	OV_RESULT res = copyOvStringToOPCUA(src->v_namespaceUri,
			&dst->namespaceUri);
	dst->serverIndex = src->v_serverIndex;
	res |= copyOvNodeIdToOPCUA(&src->p_nodeId, &dst->nodeId);
	return res;
}
...
OV_RESULT copyOvQualifiedNameToOPCUA(OV_INSTPTR_opcua_qualifiedName src,
	...
	dst->namespaceIndex = (UA_UInt16) src->v_namespaceIndex;
	return copyOvStringToOPCUA(src->v_name, &dst->name);
}
...
OV_RESULT copyOvLocalizedTextToOPCUA(OV_INSTPTR_opcua_localizedText src,
	...
	OV_RESULT res = copyOvStringToOPCUA(src->v_locale, &dst->locale);
	res |= copyOvStringToOPCUA(src->v_text, &dst->text);
	return res;
}
...
OV_RESULT createOVReferenceNodeFromOPCUA(OV_INSTPTR_ov_domain location, OV_INSTPTR_opcua_reference *newReferenceNode, OV_STRING name){
	return Ov_CreateObject(opcua_reference, *newReferenceNode, location, name);
}
/**
