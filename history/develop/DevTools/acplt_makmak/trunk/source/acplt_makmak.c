...
#error --- Betriebssystem nicht unterstuetzt --- 
	...
	if(fileExists(help)){
		...
		fprintf(fd, "# ================ Prebuild script for user libraries ==================\n");
		fprintf(fd, "#  Generated by acplt_makmak\n");
		fprintf(fd, "#\n");
		fprintf(fd, "#  You are free to edit this file, since acplt_makmak will never\n");
		fprintf(fd, "#  overwrite it\n");
		fprintf(fd, "# ======================================================================\n\n");

		fprintf(fd, "# ----------------------- CONFIGURATION START --------------------------\n");
		fprintf(fd, "#set to 1 if you want the sourcetemplate dir to be merged into the source dir\n");
		...
		fprintf(fd, "if { $argc == 1 } {\n");
		fprintf(fd, "	set libname \"$argv\"\n");
		fprintf(fd, "	set makmak \"\"\n");
		fprintf(fd, "    } elseif {$argc == 2} {\n");
		fprintf(fd, "       set libname [lindex $argv 0]\n");
		fprintf(fd, "       set makmak [lindex $argv 1]\n");
		fprintf(fd, "    } else {\n");
		fprintf(fd, "   	puts \"Usage: prebuild library_name or prebuild library_name makmak_option\"\n");
		fprintf(fd, "   	exit   \n");
		fprintf(fd, "   }\n\n");

		fprintf(fd, "while {$libname eq \"\"} {\n");
		fprintf(fd, "	puts \"Libname is empty\"\n");
		fprintf(fd, "	exit\n");
		fprintf(fd, "}\n\n");
		...
		fprintf(fd, "#check time of model editing\n");
		fprintf(fd, "if {[file exists \"../model/$libname.ovm\"]} {\n");
		fprintf(fd, "	set ovmtime [file mtime \"../model/$libname.ovm\"]\n");
		fprintf(fd, "} else {\n");
		fprintf(fd, "	set ovmtime 0\n");
		fprintf(fd, "}\n");
		fprintf(fd, "if {[file exists \"../model/$libname.ovf\"]} {\n");
		fprintf(fd, "	set ovftime [file mtime \"../model/$libname.ovf\"]\n");
		fprintf(fd, "} else {\n");
		fprintf(fd, "	set ovftime 0\n");
		fprintf(fd, "}\n");
		fprintf(fd, "if {[file exists \"../model/$libname.ovt\"]} {\n");
		fprintf(fd, "	set ovttime [file mtime \"../model/$libname.ovt\"]\n");
		fprintf(fd, "} else {\n");
		fprintf(fd, "	set ovttime 0\n");
		fprintf(fd, "}\n");
		fprintf(fd, "#does not work in windows\n");
		fprintf(fd, "#set modeltime [expr {max($ovmtime,$ovftime,$ovttime)}]\n");
		fprintf(fd, "set modeltime 0\n");
		fprintf(fd, "if {$ovmtime > $modeltime} {\n");
		fprintf(fd, "	set modeltime $ovmtime\n");
		fprintf(fd, "}\n");
		fprintf(fd, "if {$ovftime > $modeltime} {\n");
		fprintf(fd, "	set modeltime $ovftime\n");
		fprintf(fd, "}\n");
		fprintf(fd, "if {$ovttime > $modeltime} {\n");
		fprintf(fd, "	set modeltime $ovttime\n");
		fprintf(fd, "}\n\n");
		...
		fprintf(fd, "	}\n");
		fprintf(fd, "	if {[file exists \"../source/sourcetemplates\"]} {\n");
		...
		fprintf(fd, "	file rename \"../sourcetemplates\" \"../source/sourcetemplates\"\n\n");

		fprintf(fd, "	if {$error != 0} {\n");
		fprintf(fd, "		exit 1\n");
		fprintf(fd, "	}\n\n");

		fprintf(fd, "	if {$merge == 1} {\n");
		fprintf(fd, "		set dir \"../sourcetemplates\"\n");
		fprintf(fd, "		set files [glob -directory $dir *]\n");
		fprintf(fd, "		#careful merging\n");
		fprintf(fd, "		foreach f $files {\n");
		fprintf(fd, "			set filename [file tail \"$f\"]\n");
		fprintf(fd, "			if {![file exists \"../source/$filename\"]} {\n");
		fprintf(fd, "				file copy $f \"../source/$filename\"\n");
