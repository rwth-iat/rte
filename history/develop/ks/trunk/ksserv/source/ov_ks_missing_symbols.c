/* -*-c++-*- */
/* $Header: /home/david/cvs/acpltroot/runtimeserver/source/ov_ks_missing_symbols.c,v 1.2 2011-06-27 09:25:16 henning Exp $ */
/*
 ...
 */
static u_int  MemStreamGetPos(PLT_CONST XDR * xdr)
{
    return 0;
} /* MemStreamGetPos */

static bool_t MemStreamSetPos(XDR * xdr, u_int pos)
{
    return FALSE;
} /* MemStreamSetPos */
...
#ifdef KSDUMPXDRCOMMUNICATION
	PltLogStream ls;
    ls << "ks/xdrmemstream WITH DUMING !!! -- xdrmemstream_read_to_fd called with XDR  "
       << xdrs
       << " and int max "
       << *max
       << " and int err "
...
#ifdef KSDUMPXDRCOMMUNICATION
    int readfd = open("./call.xdr", O_WRONLY | O_CREAT, S_IREAD);
   if ( -1 == readfd )
      {
	PltLogStream ls2;
	ls << "CANT CREATE FILE for dumping call...";
	ls.debug();
      }
#endif
	...
	ls << "DUMPING CALL...";
	ls.debug();
        write(readfd, xdrs->x_private, count_read);
#endif
        if ( count_read < 0 ) {
            /*
...
#ifdef KSDUMPXDRCOMMUNICATION
	PltLogStream ls;
    ls << "ks/xdrmemstream WITH DUMING !!! -- xdrmemstream_write_to_fd called with XDR  "
       << xdrs
       << " and int max "
       << *max
       << " and int err "
...
#ifdef KSDUMPXDRCOMMUNICATION
    int storefd = open("./reply.xdr", O_WRONLY | O_CREAT, S_IREAD);
   if ( -1 == storefd )
      {
		PltLogStream ls2;
	ls << "CANT CREATE FILE for dumping reply...";
	ls.debug();
      }
#endif
    while ( *max != 0 ) {
	...
	ls << "DUMPING REPLY...";
	ls.debug();
        write(storefd, xdrs->x_private, count);
#endif
        if ( count_written < 0 ) {
            /*
