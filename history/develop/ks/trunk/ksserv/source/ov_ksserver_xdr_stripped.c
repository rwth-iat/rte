/*
*   $Id: ov_ksserver_xdr_stripped.c,v 1.2 2011-03-30 09:56:09 henning Exp $
*
...
*	02-Apr-2001 Ansgar M�nnemann <ansgar@plt.rwth-aachen.de>: xdr routines for OV_GETHIST.
*/
/*
*	Description:
*	------------
...
OV_BOOL ov_ksserver_xdr_bytes(
	...
	*/
	//ov_logfile_info("ov_ksserver_xdr_bytes called");
	switch(xdrs->x_op) {
	case XDR_ENCODE:
		_size = *sizep;
...
OV_BOOL ov_ksserver_xdr_string(
	...
	*/
	//ov_logfile_info("ov_ksserver_xdr_string called - debug follows");

	switch(xdrs->x_op) {
	case XDR_ENCODE:
		//ov_logfile_info("XDR ENCODE - length");
		if(sp) {
			_size = strlen(sp);
		} else {
			_size = 0;
		...
		break;
	case XDR_FREE:
		//ov_logfile_info("XDR FREE - length");
		return FALSE;
	default:
		//ov_logfile_info("XDR default - length");
		break;
	}
	if(!xdr_u_int(xdrs, &_size)) {
 		//ov_logfile_info("Cant get size");
		return FALSE;
	}
	//ov_logfile_info("Got  - length %d", _size);
	if(_size > _maxsize) {
		return FALSE;
	}
	_nodesize = _size + 1;
	...
	*/
	switch(xdrs->x_op) {
	case XDR_DECODE:
		//ov_logfile_info("XDR DECODE - data");
		if(!_nodesize) {
			return TRUE;
		}
		/*
		...
		*cpp = sp = (OV_STRING)ov_memstack_alloc(_nodesize);
		...
		/* fall into ... */
	case XDR_ENCODE:
		ret = xdr_opaque(xdrs, sp, _size);
		//ov_logfile_info("XDR ENCODE and DECODING - read data is: %s", sp);
		//return xdr_opaque(xdrs, sp, _size);
		return ret;
	default:
		//ov_logfile_info("XDR default - data");
		break;
	}
	return FALSE;
}
...
OV_BOOL ov_ksserver_xdr_array(
	...
	*/
	//ov_logfile_info("ov_ksserver_xdr_array called");
	switch(xdrs->x_op) {
	case XDR_ENCODE:
         	//ov_logfile_info("ov_ksserver_xdr_array - ENCODE length");
		_size = *sizep;
		if(!xdr_u_int(xdrs, &_size)) {
			return FALSE;
		}
		break;
	case XDR_DECODE:
         	//ov_logfile_info("ov_ksserver_xdr_array - DECODE length");
		if(!xdr_u_int(xdrs, &_size)) {
			return FALSE;
		}
		//ov_logfile_info("ov_ksserver_xdr_array - DECODE length: %d", _size);
		//ov_logfile_info("ov_ksserver_xdr_array - xdrs->private is: %s", xdrs->x_private);
		//ov_logfile_info("ov_ksserver_xdr_array - xdrs->x_handy is: %d", xdrs->x_handy);
		//ov_logfile_info("ov_ksserver_xdr_array - xdrs->x_private is: %d", xdrs->x_base);
		*sizep = _size;
		break;
	default:
         	//ov_logfile_info("ov_ksserver_xdr_array - defualt length");
		return FALSE;
	}
	_c = _size;
	if(_c>_maxsize) {
	...
	*/
	if(xdrs->x_op == XDR_DECODE) {
         	//ov_logfile_info("ov_ksserver_xdr_array - DECODE alloc memory");
		if(!_c) {
	         	//ov_logfile_info("ov_ksserver_xdr_array - NO c_ !!!");
			return TRUE;
		}
		*addrp = target = (char*)ov_memstack_alloc(_nodesize);
		if(!target) {
	         	//ov_logfile_info("ov_ksserver_xdr_array - NO target !!!");
			return FALSE;
		}
	}
	/*
...
OV_KSSERVER_DECL_XDRFNC(OV_GETVAR_ITEM) {
	...
	//ov_logfile_info("OV_KSSERVER_DECL_XDRFNC called");
	if(!ov_ksserver_xdr_OV_RESULT(xdrs, &objp->result)) {
		return FALSE;
	}
	switch(objp->result) {
...
OV_KSSERVER_DECL_XDRFNC(OV_GETVAR_PAR) {
	//ov_logfile_info("OV_KSSERVER_DECL_XDRFNC with GETVAR called");
