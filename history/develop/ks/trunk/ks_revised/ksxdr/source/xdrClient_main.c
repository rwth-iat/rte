...
OV_DLLFNCEXPORT OV_RESULT ksxdr_xdrClient_serverHost_set(
    OV_INSTPTR_ksxdr_xdrClient          pobj,
    ...
    {	/*	new host	*/
    	...
    	result = getChannelPointer(pobj, &pChannel, &pVtblChannel);
    	if(Ov_Fail(result))
    		return result;
...
OV_DLLFNCEXPORT OV_RESULT ksxdr_xdrClient_serverName_set(
    OV_INSTPTR_ksxdr_xdrClient          pobj,
    const OV_STRING  value
) {
    OV_RESULT					result;
    ...
    {	/*	new host	*/
    	if(ov_string_compare(value, "MANAGER") == OV_STRCMP_EQUAL)
    	{
    		/*	reset serverPort	*/
    		result = ov_string_setvalue(&(pobj->v_serverPort), pobj->v_ManagerPort);
    		if(Ov_Fail(result))
    			return result;
    	}
    	else
