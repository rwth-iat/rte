...
void ksservtcp_tcpclient_typemethod(
	...
	if((err > 0) && FD_ISSET(receivesocket, &read_flags))
	{
		...
		default:	//no manager command
			...
			if(size_return == 28)  //no body generated yet --> error --> sending generic error
			{
				headlength = 48;
				header[43] = 0xff;
				header[47] = KS_ERR_GENERIC;

				ov_logfile_error("tcpclient - no body generated by ovks, most probably corrupted xdr received");
				xdrlret = 44;
			}
			else
			{
			...
			do
			{
				...
				sentChunkSize = send(receivesocket, placeInBuffer, headlength, 0);
				if (sentChunkSize == -1)
				{
