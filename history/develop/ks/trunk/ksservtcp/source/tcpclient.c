...
void ksservtcp_tcpclient_typemethod(
		...
		do {//read KS command from socket in chunks until nothing more appears
			...
			if(buffer == 0) {
				...
				if ((IOCTL_SOCKET(receivesocket, FIONBIO, (char*) &on)) == -1) {
					perror("ioctl(tcpclient) failed (set to non-blocking on error)");
				}
				return;
			}
			...
			//prevent blocking if ks-command has exact size of n CHUNKS
			if(recvBytes == BUFFER_CHUNK_SIZE)
			{
				FD_ZERO(&read_flags);
				FD_SET(receivesocket, &read_flags); // get read flags
				waitd.tv_sec = 0;     // Set Timeout
				waitd.tv_usec = 10;    //  10 usecs only since the rest of a command is already in the buffer, if there is a rest
				err = select(receivesocket + 1, &read_flags, (fd_set*) 0, (fd_set*)0,&waitd);
				if(err <= 0)		//there is no rest of a command; stop receiving
					break;
			}
	...
	//					size_return = (&resultXDRs)->x_handy+28;
		...
		//to set non-blocking
		//wait untill write commands are done
		FD_ZERO(&write_flags);
		FD_SET(receivesocket, &write_flags); // get write flags
		waitd.tv_sec = 0;     // Set Timeout
		waitd.tv_usec = 1000;    //  1 millisecond
		...
		if(err < 0)
		{
			perror("tcpclient: error waiting for running send-commands:");
		}
	}
	...
	//set non-blocking
	if ((IOCTL_SOCKET(receivesocket, FIONBIO, (char*) &on)) == -1) {
		perror("ioctl(tcpclient) failed (set to non-blocking at end)");
	}
	return;
}
