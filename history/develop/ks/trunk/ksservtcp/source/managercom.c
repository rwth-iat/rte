...
OV_DLLFNCEXPORT void ksservtcp_managercom_startup(OV_INSTPTR_ov_object pobj) {
	...
	if (getenv("MNGPORT")) {
		...
		ksservtcp_managercom_getmngport(pinst);
		ksserv_logfile_info("mngport: %d", pinst->v_mngport);
	}
...
/**
 * This method gets the port of the ks_manager from
 * the portmapper.
 */
OV_DLLFNCEXPORT void ksservtcp_managercom_getmngport(
		OV_INSTPTR_ksservtcp_managercom pobj) {
	OV_INSTPTR_ksservtcp_managersendrecv
			pmsr =
					(OV_INSTPTR_ksservtcp_managersendrecv) Ov_SearchChild(ov_containment,pobj, "managersendrecv");
	...
	//if we are running w/o portmapper, 7509 is
	//defined to be our ksmanager
	...
	hp = gethostbyname("127.0.0.1");
	if (hp == 0) {
		ksserv_logfile_info("Unknown host (ksservtcp)");
		CLOSE_SOCKET(sock);
		pobj->v_socket = -1;
		return;
	}
	...
	free(pobj->v_xdr);
	if (bytes < 0) {
		ksserv_logfile_info("send(getmngport) failed");
		CLOSE_SOCKET(sock);
		pobj->v_socket = -1;
		return;
	}
	pobj->v_status = STPMRQSND;
	//~ ksserv_logfile_info("send(getmngport) ok");
	...
	//~ ksserv_logfile_info("recv(mngport) ok");
