...
OV_DLLFNCEXPORT void ksapi_getEPidentifiers_submit(
	...
	if((pobj->v_host) && (pobj->v_path)&& (pobj->v_namemask) && (ksapi_KSCommon_status_get(pksc) != STATUS_KSCOMMON_BUSY)){
		...
		generategetepxdr(&xdr, &xdrlength, pobj->v_path, (KS_OT_DOMAIN | KS_OT_LINK | KS_OT_VARIABLE), pobj->v_namemask, KS_EPF_DEFAULT);		//Don't ask for histories, since we can't parse the answer yet
