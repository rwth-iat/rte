...
/*	ksapi_linkObject_linkedpath_get
...
/*	ksapi_linkObject_linkedpath_set
...
OV_DLLFNCEXPORT void ksapi_linkObject_startup(
	...
	//OV_INSTPTR_ksapi_linkObject sobj = Ov_StaticPtrCast(ksapi_linkObject, pobj);
...
OV_DLLFNCEXPORT void ksapi_linkObject_submit(
	...
	if((pobj->v_host) && (pobj->v_server) && (pobj->v_linkedpath) && (pobj->v_path) && (ksapi_KSCommon_status_get(pksc) != STATUS_KSCOMMON_BUSY)){
		...
		//send
		generatelinkobjectxdr(&xdr, &xdrlength, pobj->v_path, pobj->v_linkedpath, pobj->v_position, pobj->v_element);
		ksapi_Channel_sendxdr(channel, pksc, xdr, xdrlength);
	
	}
	else if(ksapi_KSCommon_status_get(pksc) == STATUS_KSCOMMON_BUSY)
	{
		log_error("linkObject: connection in use");
...
OV_DLLFNCEXPORT void ksapi_linkObject_setandsubmit(
	...
	if (Ov_Fail(ksapi_linkObject_linkedpath_set(pobj, linkedpath))) {
		log_error("link: error setting linkedpath");
		return;
	}
	if (Ov_Fail(ksapi_linkObject_position_set(pobj, position))) {
		log_error("linkObject: error setting position");
		return;
	}
	if (Ov_Fail(ksapi_linkObject_element_set(pobj, element))) {
		log_error("linkObject: error setting element");
		return;
	}
	ksapi_linkObject_submit(pobj);
	return;
}

OV_DLLFNCEXPORT void ksapi_linkObject_returnMethodxdr(
	...
	//Analyse xdr
	analyselinkobjectreply(xdr, xdrlength);
