...
\section{Die Bibliothek \emph{ksapi}}
	...
	\subsection{getEPidentifiers}
		Die Klasse \emph{getEPidentifiers} gibt eine Liste der Identifier aller unter dem angegebenen Pfad sich befindenden Kinder an. Bei den kindern kann es sich um Domains, Variablen und Links handeln. Die Klasse liest die EP-Objekte als ganzes aus, gibt aber nur die Identifier zurück. Sollten andere Attribute benötigt werden, muss die Return-Methode entsprechend angepasst werden. In der Return-Methode sind die relevanten Daten im XDR-Strom kommentiert.
		\subsubsection{Welche Einstellungen kann man vornehmen?}
			Zum einen können natürlich der Host, der Server und der Pfad der Domain, deren Kinder man lesen möchte eingestellt werden. Dies kann entweder über die Set-Funktionen der einzelnen Variablen geschehen oder durch Parameterübergabe an die entsprechende Funktion \textsl{setandsubmit}.\\
			Alle Änderungen der Variablen dürfen nur über die entsprechenden Set-Methoden vorgenommen werden; bei der Parameterübergabe wird dies intern gemacht. Wenn die Variablen direkt gesetzt werden, werden eventuell wichtige Aufräumaktionen nicht durchgeführt.
	...
	\begin{center}
		\begin{tabular}{|l|l|l|}
			...
			\hline
			EPIdentifiers& getEPidentifiers&setandsubmit(pobj, host, server, path)\\
			\hline
	...
	\subsection{createObject}
		\subsubsection{Welche Einstellungen kann man vornehmen?}
		     Zum einen können natürlich der Host, der Server und der Pfad des zu erzeugenden Objektes eingestellt werden sowie der Pfad der zu erzeugenden Klasse (librarypath) und die Position in der Hierarchie (position, element). Dies kann entweder über die Set-Funktionen der einzelnen Variablen geschehen oder durch Parameterübergabe an die entsprechende Funktion \textsl{setandsubmit}.\\
			Alle Änderungen der Variablen dürfen nur über die entsprechenden Set-Methoden vorgenommen werden; bei der Parameterübergabe wird dies intern gemacht. Wenn die Variablen direkt gesetzt werden, werden eventuell wichtige Aufräumaktionen nicht durchgeführt.
		\subsubsection{Wie starte ich den Sendevorgang?}
	...
	\subsection{renameObject}
		\subsubsection{Welche Einstellungen kann man vornehmen?}
		     Zum einen können natürlich der Host, der Server und der Pfad des umzubenennenden Objektes eingestellt werden sowie der neue Pfad unter dem das Objekt angelegt werden soll. Dies kann entweder über die Set-Funktionen der einzelnen Variablen geschehen oder durch Parameterübergabe an die entsprechende Funktion \textsl{setandsubmit}.\\
			Alle Änderungen der Variablen dürfen nur über die entsprechenden Set-Methoden vorgenommen werden; bei der Parameterübergabe wird dies intern gemacht. Wenn die Variablen direkt gesetzt werden, werden eventuell wichtige Aufräumaktionen nicht durchgeführt.
		\subsubsection{Wie starte ich den Sendevorgang?}
		     Auch hier muss zuerst eine Rückgabemethode definiert werden. Danach gibt es zwei Möglichkeiten, den Sendevorgang zu starten:
			\begin{enumerate}
	...
	\subsection{deleteObject}
		\subsubsection{Welche Einstellungen kann man vornehmen?}
		     Es können der Host, der Server und der Pfad des zu löschenden Objektes eingestellt werden. Dies kann entweder über die Set-Funktionen der einzelnen Variablen geschehen oder durch Parameterübergabe an die entsprechende Funktion \textsl{setandsubmit}.\\
			Alle Änderungen der Variablen dürfen nur über die entsprechenden Set-Methoden vorgenommen werden; bei der Parameterübergabe wird dies intern gemacht. Wenn die Variablen direkt gesetzt werden, werden eventuell wichtige Aufräumaktionen nicht durchgeführt.
		\subsubsection{Wie starte ich den Sendevorgang?}
		     Auch hier muss zuerst eine Rückgabemethode definiert werden. Danach gibt es zwei Möglichkeiten, den Sendevorgang zu starten:
			\begin{enumerate}
     ...
     \subsection{linkObject}
		\subsubsection{Welche Einstellungen kann man vornehmen?}
		     Zum einen können natürlich der Host, der Server und der Pfad des zu verlinkenden Objektes eingestellt werden sowie der Pfad des Objektes mit dem es verlinkt werden soll (linkedpath) und die Position in der Hierarchie (position, element). Dies kann entweder über die Set-Funktionen der einzelnen Variablen geschehen oder durch Parameterübergabe an die entsprechende Funktion \textsl{setandsubmit}.\\
			Alle Änderungen der Variablen dürfen nur über die entsprechenden Set-Methoden vorgenommen werden; bei der Parameterübergabe wird dies intern gemacht. Wenn die Variablen direkt gesetzt werden, werden eventuell wichtige Aufräumaktionen nicht durchgeführt.
		\subsubsection{Wie starte ich den Sendevorgang?}
		     Auch hier muss zuerst eine Rückgabemethode definiert werden. Danach gibt es zwei Möglichkeiten, den Sendevorgang zu starten:
			\begin{enumerate}
	...
	\subsection{unlinkObject}
		\subsubsection{Welche Einstellungen kann man vornehmen?}
		     Zum einen können natürlich der Host, der Server und der Pfad des zu unlinkenden Objektes eingestellt werden sowie der Pfad des Objektes mit dem es verlinkt ist (linkedpath). Dies kann entweder über die Set-Funktionen der einzelnen Variablen geschehen oder durch Parameterübergabe an die entsprechende Funktion \textsl{setandsubmit}.\\
			Alle Änderungen der Variablen dürfen nur über die entsprechenden Set-Methoden vorgenommen werden; bei der Parameterübergabe wird dies intern gemacht. Wenn die Variablen direkt gesetzt werden, werden eventuell wichtige Aufräumaktionen nicht durchgeführt.
		\subsubsection{Wie starte ich den Sendevorgang?}
		     Auch hier muss zuerst eine Rückgabemethode definiert werden. Danach gibt es zwei Möglichkeiten, den Sendevorgang zu starten:
			\begin{enumerate}
...
\label{var}
\begin{center}
	\begin{tabular}{|l|p{6.1cm}|p{5cm}|}
		\hline
		\textsl{Variable}&Bedeutung&\textsc{Standardwert}\\
		\hline
		\textsl{send*}& Hier kann der Wert der Variablen \linebreak eingetragen werden, die versendet werden soll.(nur bei set)& verschiedene Werte\\
		\hline
		\textsl{received*}& Enthält den Wert der gelesenen Variable (nur bei get).& NULL\\
		\hline
		\textsl{host}& Der Host, auf dem der Receiver \linebreak ausgeführt wird.& \textsc{127.0.0.1}\\
		\hline
		\textsl{server}&Der Server, auf dem der Receiver \linebreak ausgeführt wird.& \textsc{fb\_testlib}\\
		\hline
		\textsl{path}&Der Pfad, wo der Receiver auf dem \linebreak Server liegt.&\textsc{/TechUnits/client.*}\\
		\hline
		\textsl{librarypath}&Der Pfad der zu erzeugenden Klasse (nur bei createObject).&\textsc{/Libraries/*}\\
		\hline
		\textsl{newpath}&Der Pfad, auf den das Objekt \linebreak umbenannt werden soll (nur bei renameObject).&\textsc{/TechUnits/*}\\
		\hline
		\textsl{linkedpath}&Der Pfad des Objektes mit dem verlinkt werden soll (nur bei (un)linkObject).&\textsc{/TechUnits/client.*}\\
		\hline
		\textsl{position}&Position in der Hierarchie (nur bei \linebreak createObject und linkObject).&\textsc{0=default, 1=Anfang, 2=Ende, 3=bevor, 4=nach}\\
		\hline
		\textsl{element}&Der Pfad, auf den sich \textsl{position} bezieht (nur bei createObject und linkObject).&\textsc{/TechUnits/*}\\
		\hline
		...
		\hline
		\textsl{send}&Wird auf true gesetzt, wenn der \linebreak Kommunikationsvorgang gestartet werden soll.& \textsc{False}\\
		\hline
