...
#if DBG_TO_STDERR
fprintf(stderr, "ml_reorde_block %lu mit %lu Bytes\n", (unsigned long)(__ml_byte_t*)ptrLst, size);
#endif

if( (((unsigned long)(__ml_byte_t*)ptrLst - ML_STARTADDRESS) % BLOCKSIZE) != 0 ) {
#if DBG_TO_STDERR
    fprintf(stderr, "!!! Adresse  %lu nicht an der Block-Ausrichtung (start: %lu) !!!\n", (unsigned long)(__ml_byte_t*)ptrLst, ML_STARTADDRESS);
#endif
    return;
}
